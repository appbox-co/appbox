...
export function ClientAppConnections({
  ...
  useEffect(() => {
    queueMicrotask(() =>
      setCurvatures(
        Array.from({ length: appRefs.length }, () => Math.random() * 40 - 20)
      )
    )
  }, [appRefs.length])
  useEffect(() => {
    if (!isClient || !showBeams || !inView || appRefs.length === 0) return
    const check = () => {
      if (
        containerRef.current &&
        centerRef.current &&
        appRefs.every((r) => r.current)
      ) {
        queueMicrotask(() => setRefsReady(true))
      }
    }
    check()
    const id = requestAnimationFrame(check)
    return () => cancelAnimationFrame(id)
  }, [isClient, showBeams, inView, appRefs.length, appRefs])
